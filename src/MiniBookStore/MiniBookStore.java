...
public class MiniBookStore {
    ...
    //1-product, book, notebook claslarını oluştur
    public static <ProductService> void enter(){
        //10-kategori menü
        Scanner inp=new Scanner(System.in);
        System.out.println("--- Mimi Book Store ---");
        int select=-1;
        while (select!=0){
            System.out.println(" Ürün Yönetim Paneli ");
            System.out.println("1-Kitaplar");//bookservice lazım
            System.out.println("2-Defterler");//notebookservice lazım
            System.out.println("0-Çıkış");
            System.out.println("Seçiminiz:");
            select=inp.nextInt();
            //ProductService i referans alarak service
            ProductService service=select==1 ? (ProductService) new BookService() : (ProductService) new NotebookService();
